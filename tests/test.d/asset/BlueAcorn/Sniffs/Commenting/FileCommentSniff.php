...
class BlueAcorn_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        foreach ($tokens[$commentStart]['comment_tags'] as $tag) {
            ...
            if ($name === '@version') {
                if (version_compare($tokens[$string]['content'], '1.0.0') < 0) {
                    $error = 'Expected version at least 1.0.0';
                    $fix   = $phpcsFile->addFixableError($error, $tag, 'IncorrectVersion');
                }
            } else if ($name === '@author') {
                if ($tokens[$string]['content'] !== 'Blue Acorn, Inc. <code@blueacorn.com>') {
                    $error = 'Expected "Blue Acorn, Inc. <code@blueacorn.com>" for author tag';
                    $fix   = $phpcsFile->addFixableError($error, $tag, 'IncorrectAuthor');
                    if ($fix === true) {
                        $expected = 'Blue Acorn, Inc. <code@blueacorn.com>';
                ...
                if ($tokens[$string]['content'] !== "Copyright © {$year} Blue Acorn, Inc") {
                    ...
                    if ($fix === true) {
                        $expected ="Copyright © {$year} Blue Acorn, Inc";
                        $phpcsFile->fixer->replaceToken($string, $expected);
                    }
                }
            }//end if
