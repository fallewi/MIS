...
env_bootstrap(){
  ...
  ENV_DIR="$REPO_ROOT/env/$ENV"
  
  [ -d "$ENV_DIR" ] || error "$ENV environment not found"
  [ -e "$ENV_DIR/ssh_config" ] || error "$ENV environment has no ssh_config" 
}
...
env_bootstrap_ansible(){
  ...
  export ANSIBLE_SSH_CONF_FILE="$ENV_DIR/ssh_config"
  export ANSIBLE_SSH_CONF_ENVARS="$REPO_ROOT/env/envars,$ENV_DIR/envars"
