 ...
 * Last Modified: 2016-05-30T12:17:32+02:00
...
class Xtento_StockImport_Model_Import_Entity_Stock extends Xtento_StockImport_Model_Import_Entity_Abstract
{
    ...
    private function _insertStockItem($productId, $productIdentifier, $updateData)
    {
        ...
        if (Mage::helper('xtcore/utils')->isExtensionInstalled('MDN_AdvancedStock')) {
            if ($updateData['stock_id'] > 0 && $productId > 0) {
                $newQty = $updateData['qty'];
                $previousQty = $this->_stockItems[$updateData['stock_id']][$productId]['qty'];
                $qtyForStockMovement = $newQty - $previousQty;
                if ($qtyForStockMovement >= 0) {
                    $sourceStockId = 0;
                    $targetStockId = $updateData['stock_id'];
                } else {
                    $sourceStockId = $updateData['stock_id'];
                    $targetStockId = 0;
                    $qtyForStockMovement = abs($qtyForStockMovement);
                }
                if ($qtyForStockMovement > 0) {
                    $this->_writeAdapter->insert(
                        $this->getTableName('AdvancedStock/StockMovement'),
                        array(
                            'sm_product_id' => $productId,
                            'sm_qty' => $qtyForStockMovement,
                            'sm_coef' => '0',
                            'sm_description' => sprintf(
                                'Stock adjustment from Xtento Import - %s',
                                $qtyForStockMovement
                            ),
                            'sm_type' => 'adjustment',
                            'sm_date' => date('Y-m-d H:i:s'),
                            'sm_source_stock' => $sourceStockId,
                            'sm_target_stock' => $targetStockId
                        )
                    );

                    $updatedFieldsProduct['updated_at'] = date('Y-m-d H:i:s');
                    $this->_writeAdapter->update(
                        $this->getTableName('catalog/product'),
