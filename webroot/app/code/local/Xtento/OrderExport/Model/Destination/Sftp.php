 ...
 * Last Modified: 2019-02-25T14:16:02+01:00
...
class Xtento_OrderExport_Model_Destination_Sftp extends Xtento_OrderExport_Model_Destination_Abstract
{
    ...
    public function initConnection()
    {
        ...
        if (Mage::helper('xtcore/utils')->mageVersionCompare(Mage::getVersion(), '1.9.4.0', '>=')
            || (version_compare(PHP_VERSION, '7.0.0', '>=') && @class_exists('phpseclib\Net\SFTP'))) {
            // PHP 7.2 compatibility
            if (!class_exists('phpseclib\Net\SFTP', false)) {
                require_once('phpseclib/Net/SFTP.php');
            }
            if (class_exists('phpseclib\Net\SFTP')) {
                $this->_connection = new phpseclib\Net\SFTP($this->getDestination()->getHostname(), $this->getDestination()->getPort(), $this->getDestination()->getTimeout());
            } else {
