 ...
 * Last Modified: 2019-03-27T10:47:59+01:00
...
class Xtento_OrderExport_Model_Export extends Mage_Core_Model_Abstract
{
    ...
    public function eventExport($filters, $forcedCollectionItem = false)
    {
        ...
        if (empty($generatedFiles) && ($this->getLogEntry()->getResult() === NULL || $this->getLogEntry()->getResult() === Xtento_OrderExport_Model_Log::RESULT_SUCCESSFUL)) {
    ...
    {
        if (!Mage::helper('xtento_orderexport')->getModuleEnabled()) {
            return $this;
        }
    ...
    protected function _runExport($filters, $forcedCollectionItem = false)
    {
        try {
            @set_time_limit(0);
            ...
            if (preg_match('/\|merge\:/', $this->getProfile()->getXslTemplate())) { // Merge multiple profiles. Format: filename|merge:1,3,4,5 (<- profile ids)
                ...
                if ($this->getProfile()->getExportOneFilePerObject()) {
                    ...
                    foreach ($this->getReturnArrayWithObjects() as $returnObject) {
                        $generatedFiles += Mage::getModel('xtento_orderexport/output_' . $type, array('profile' => $this->getProfile()))->convertData(array($returnObject));
                    }
                } else {
    ...
    protected function _exportObjects($filters, $forcedCollectionItem = false)
    {
    ...
    protected function _adjustOrderStatus()
    {
        $statusUpdated = false;
    ...
    protected function _invoiceShipOrder()
    {
        if ($this->getProfile()->getEntity() == self::ENTITY_ORDER) {
    ...
    protected function _changeOrderStatus($newStatus)
    {
        if ($newStatus == '') {
    ...
    protected function _errorEmailNotification()
    {
        ...
        if ($this->getLogEntry()->getResult() >= Xtento_OrderExport_Model_Log::RESULT_WARNING) {
            try {
                ...
                $mail->setSubject('Magento Order Export Module @ ' . $serverName);
