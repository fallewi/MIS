 ...
 * Last Modified: 2019-05-21T11:22:40+02:00
...
class Xtento_OrderExport_Model_Export_Data_Shared_Items extends Xtento_OrderExport_Model_Export_Data_Abstract
{
    ...
    public function getExportData($entityType, $collectionItem)
    {
        ...
        foreach ($items as $item) {
            ...
            // $parentItem = Mage::getModel('sales/order_creditmemo_item')->load($parentItem->getOrderItemId(), 'order_item_id’);
            ...
            // Third party extensions - item data
            // SafeMage_MultiInventory
            if (Mage::helper('xtcore/utils')->isExtensionInstalled('SafeMage_MultiInventory')) {
                $this->_writeArray['safemage_multiinventory'] = array();
                $this->_writeArray = &$this->_writeArray['safemage_multiinventory'];
                $readAdapter = Mage::getSingleton('core/resource')->getConnection('core_read');
                $orderItemId = $item->getOrderItemId() ? $item->getOrderItemId() : $item->getId();
                $dataRow = $readAdapter->fetchRow("SELECT * FROM " . Mage::getSingleton('core/resource')->getTableName('safemage_multiinventory_warehouse_order_item') . " WHERE order_item_id = " . $readAdapter->quote($orderItemId));
                if (is_array($dataRow)) {
            ...
            if (($this->fieldLoadingRequired('parent_item') || $this->fieldLoadingRequired('products_total_cost') || $this->fieldLoadingRequired('product_total_cost')) && $parentItem) {
                ...
                // Parent Item Enterprise Gift Wrapping information
                if ($this->fieldLoadingRequired('enterprise_giftwrapping') && Mage::helper('xtcore/utils')->getIsPEorEE()) {
                    if ($parentItem->getGwId()) {
                        $this->_writeArray['enterprise_giftwrapping'] = array();
                        $this->_writeArray =& $this->_writeArray['enterprise_giftwrapping'];
                        $wrapping = Mage::getModel('enterprise_giftwrapping/wrapping')->load($parentItem->getGwId());
                        if ($wrapping->getId()) {
                            foreach ($wrapping->getData() as $key => $value) {
                                $this->writeValue($key, $value);
                            }
    ...
    protected function _writeCustomOptions($options, &$writeArray, $object, $productId = null)
    {
    ...
    protected function _writeProductAttributes($object, $item, $isParentItem = false)
    {
        ...
        if (isset($this->_cache['product_attributes'][$object->getStoreId()]) && isset($this->_cache['product_attributes'][$object->getStoreId()][$item->getProductId()])) {
            ...
            if ($this->fieldLoadingRequired('_total_cost')) {
                ...
                if ($product->getId()) {
                    if (!$isParentItem) $this->_totalCost += ($product->getCost() * $item->getQtyOrdered());
