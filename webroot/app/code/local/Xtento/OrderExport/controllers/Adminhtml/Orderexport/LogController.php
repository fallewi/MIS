 ...
 * Last Modified: 2018-09-27T16:02:56+02:00
...
class Xtento_OrderExport_Adminhtml_OrderExport_LogController extends Xtento_OrderExport_Controller_Abstract
{
    ...
    private function _getFilesForLogId($logId, $massDownload = false)
    {
        ...
        if (empty($savedFiles)) {
            if (!$massDownload) Mage::getSingleton('adminhtml/session')->addWarning(Mage::helper('xtento_orderexport')->__('There is nothing to download. No files have been saved with this export, or you disabled "Save local copies of exports" in the profile. (Log ID: %d)', $logId));
            return false;
        }
