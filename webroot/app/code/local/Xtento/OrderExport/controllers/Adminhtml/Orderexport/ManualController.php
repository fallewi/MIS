 ...
 * Last Modified: 2017-03-06T15:50:19+01:00
    ...
    public function manualPostAction()
    {
        ...
        if (!$profile->getId()) {
            if ($this->getRequest()->getPost('start_download', false)) {
                ...
                Mage::getModel('core/cookie')->set('lastErrorMessage', Mage::helper('xtento_orderexport')->__('No profile selected or this profile does not exist anymore.'), null, '/', '', null, false);
                Mage::getModel('core/cookie')->set('lastMessage', false, null, '/', '', null, false);
                return $this;
            } else {
                Mage::getSingleton('adminhtml/session')->addWarning(Mage::helper('xtento_orderexport')->__('No profile selected or this profile does not exist anymore.'));
                return $this->_redirectReferer();
            }
        }
        // Table prefix
        ...
        if ($this->getRequest()->getPost('increment_from') !== NULL) {
            ...
            if ($object && $object->getId()) {
                ...
                if ($this->getRequest()->getPost('increment_from') != 1) {
                    if ($this->getRequest()->getPost('start_download', false)) {
                        Mage::getModel('core/cookie')->set('fileDownload', 'true', null, '/', '', null, false);
                        Mage::getModel('core/cookie')->set('lastErrorMessage', Mage::helper('xtento_orderexport')->__('The supplied starting increment_id does not exist. Use 1 to export from the beginning.'), null, '/', '', null, false);
                        Mage::getModel('core/cookie')->set('lastMessage', false, null, '/', '', null, false);
                        return $this;
                    } else {
                        Mage::getSingleton('adminhtml/session')->addWarning(Mage::helper('xtento_orderexport')->__('The supplied starting increment_id does not exist. Use 1 to export from the beginning.'));
