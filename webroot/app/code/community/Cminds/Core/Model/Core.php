...
class Cminds_Core_Model_Core extends Mage_Core_Model_Config_Data
{
    ...
    public static $confExtensions = array(
        ...
        'cmindsConf_cminds_supplierfrontendproductuploader' => array(
            'extension_name' => 'Cminds_Supplierfrontendproductuploader',
            'related_to' => 'Cminds_Marketplace'
        ),
...
//        CURLOPT_SSLVERSION
    ...
    private function apiCall($action = '')
    {
        ...
        $iClient->setUri($url)
            ...
            ->setConfig(array(
                'timeout' => 15,
                'sslverify' => false,
                'curloptions' => array('CURLOPT_SSLVERSION' => $this->sslVersion)
            ))
        ...
        if ($result === false) {
            ...
            if (isset($result->error)) {
                if ($result->error === self::NO_ACTIVATIONS_STATUS) {
                    ...
                    Mage::getModel('adminnotification/inbox')->add(4,
                        Mage::helper('cminds')->__('%s %s was activation failed !', $this->itemName,
                            (string)Mage::getConfig()->getModuleConfig($this->moduleName)->version),
                        Mage::helper('cminds')->__('You have exceeded the number of the approved licenses for the %s. The extension will not work until the license issue is resolved.',
                            $this->itemName));
                } else {
                    if ($result->license === 'invalid') {
                        Mage::getModel('adminnotification/inbox')->add(4,
                            Mage::helper('cminds')->__('%s %s was activation failed !', $this->itemName,
                                (string)Mage::getConfig()->getModuleConfig($this->moduleName)->version),
                            Mage::helper('cminds')->__('%s %s license key is not correct. Please enter a valid license key.',
                                $this->itemName,
                                (string)Mage::getConfig()->getModuleConfig($this->moduleName)->version));
                ...
                if ($result->license === 'invalid') {
                    Mage::getModel('adminnotification/inbox')->add(4,
                        Mage::helper('cminds')->__('%s %s was activation failed !', $this->itemName,
                            (string)Mage::getConfig()->getModuleConfig($this->moduleName)->version),
                        Mage::helper('cminds')->__('%s %s license key is not correct. Please enter a valid license key.',
                            $this->itemName, (string)Mage::getConfig()->getModuleConfig($this->moduleName)->version));
                } else {
                    Mage::getModel('adminnotification/inbox')->add(4,
                        Mage::helper('cminds')->__('%s %s was activated successfully !', $this->itemName,
                            (string)Mage::getConfig()->getModuleConfig($this->moduleName)->version),
                        Mage::helper('cminds')->__('%s %s is available now. Please check system configuration to finish configuration',
                            $this->itemName, (string)Mage::getConfig()->getModuleConfig($this->moduleName)->version));
                }
            ...
            Mage::getModel('core/config')->saveConfig($this->optionCountLicenseMaxActivations,
                (int)$result->license_limit);
    ...
    public function deactivateLicense()
    {
        ...
        if ($result === false) {
            ...
            if (isset($result->error)) {
                ...
                Mage::getModel('adminnotification/inbox')->add(4,
                    Mage::helper('cminds')->__('%s %s was deactivated successfully !', $this->itemName,
                        (string)Mage::getConfig()->getModuleConfig($this->moduleName)->version),
                    Mage::helper('cminds')->__('THe extension is deactivated now.', $this->itemName,
                        (string)Mage::getConfig()->getModuleConfig($this->moduleName)->version));
    ...
    public function isLicenseOk()
    {
        ...
        $licenseOk = !empty($this->license) && in_array($this->licenseStatus,
                array('valid')) && $isLicenseActivationCountOk;
        return $licenseOk;
    }
        ...
        foreach (self::$items AS $i => $extension) {
            ...
            if ($this->isLicenseOk()) {
                ...
                if (version_compare($result->new_version, $installedVersion)) {
                    Mage::getModel('adminnotification/inbox')->add(4,
                        Mage::helper('cminds')->__('%s update is available', $this->itemName),
                        Mage::helper('cminds')->__('%s %s is available. Login to your customer panel and download lastest version !',
                            $this->itemName, $result->new_version), $this->extensionPage, false);
                }
            } else {
