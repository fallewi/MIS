...
class Cminds_MultiUserAccounts_Model_Sales_Order extends Mage_Sales_Model_Order
{
    public function queueNewOrderEmail($forceMode = false)
    {
        ...
        switch ($emailTo) {
            ...
            case Cminds_MultiUserAccounts_Model_SubAccount_Emails::EMAIL_SUBACCOUNT:
                if ($subAccountId = $this->getSubaccountId()) {
                    $subAccountData = $helper->getSubAccountData($subAccountId);
                    if ($subAccountData['get_order_email'] == 1) {
                ...
                if (Mage::app()->getStore()->isAdmin()) {
                    ...
                    if (!$subAccount = $helper->isSubAccountMode()) {
                        ...
                        if (!$helper->ifShareSession()) {
                            ...
                            $emailInfo->addTo($this->getCustomerEmail(), $customerName);
                            $subAccountData = $helper->getSubAccountData($subAccount->getId());
                            if ($subAccountData['get_order_email'] == 1) {
                                $emailInfo->addTo($subAccountData['email'], $customerName);
                            }
                        }
