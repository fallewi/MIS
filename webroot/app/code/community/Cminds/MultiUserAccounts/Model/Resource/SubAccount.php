...
class Cminds_MultiUserAccounts_Model_Resource_SubAccount extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    protected function _beforeSave(Mage_Core_Model_Abstract $subAccount)
    {
        ...
        if (!Mage::registry('converting_customer') && $this->_checkUniqueEmail($subAccount)) {
...
//         set confirmation key logic
    ...
    protected function _checkUniqueEmail($subAccount)
    {
        ...
        $customerCheck = $this->_getCheckUniqueEmailResult($accountResource->getEntityTable(),
            $accountResource->getEntityIdField(), $subAccount);
        $subAccountCheck = $this->_getCheckUniqueEmailResult($this->getMainTable(), $this->getIdFieldName(),
            $subAccount, $subAccount->getId());
    ...
    protected function _getCheckUniqueEmailResult($table, $idField, $subAccount, $idCheck = null)
    {
        ...
        //*
        if ($subAccount->getData('customer_id') != null) {
            $bind['customer_id'] = (int)$subAccount->getData('customer_id');
            $select->where('entity_id != :customer_id');
        }
