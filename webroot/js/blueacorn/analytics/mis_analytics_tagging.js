...
(function($) {
    ...
    $(document).ready(function() {
        ...
        //Featured products
        $('body.cms-home .feature-wrap div.feature-item').each(function(){
            var $this = $(this),
        ...
        //Hero button.
        $('body.cms-home div.hero-content > a.button').on('mousedown', function() {
        ...
        $('body.catalog-product-view .col-main').on('mousedown', 'span.yotpo-display-wrapper .write-review-button', function() {
        ...
        $('body.catalog-product-view .product-img-box .thumb-link').each(function() {
            var $this = $(this),
                label = appendUrl($('.top-product-details div.product-name').text().trim());
        ...
        $('body.catalog-product-view #product-options-wrapper select').change(function() {
            var $this = $(this),
                pageEl = $this.children('option:first-child').text().trim().toLowerCase(),
                pageElParts = pageEl.split(" "),
                index = this.selectedIndex,
                label = appendUrl($.trim($this.find('option').eq(index).html()));

            pageEl = pageElParts[0].capitalizeString() + " " + pageElParts[1].capitalizeString();

            BA_GAQ.trackEvent('Product Page', pageEl, label);
        });

        //Product Page: add to cart wrapper button and links
        $('body.catalog-product-view .add-to-cart-wrapper').on('mousedown', 'button, a', function() {
            var $this   = $(this),
                pageEl,
        ...
        //Compare Page: All
        jQuery('body.catalog-product-compare-index table.compare-table').each(function() {
            var $this = jQuery(this),
                removeRow = $this.find('tr td a.btn-remove'),
                productRowName = $this.find('tr.product-shop-row.top td h2.product-name a'),
                productRowImage = $this.find('tr.product-shop-row.top td a.product-image'),
                topRowButton = $this.find('tr.product-shop-row.bottom button.btn-cart'),
                topRowWishlist = $this.find('tr.product-shop-row.bottom a.link-wishlist'),
                bottomRowButton = $this.find('tr.add-to-row button.btn-cart'),
                bottomRowWishlist = $this.find('tr.add-to-row a.link-wishlist');

            productRowName.each(function(index) {
                var label = appendUrl(jQuery(this).text().trim()),
                    els = $([removeRow[index], productRowName[index], productRowImage[index], topRowButton[index], topRowWishlist[index], bottomRowButton[index], bottomRowWishlist[index]]);
                ...
                els.map(function(){
                    ...
                    if (row.hasClass('bottom')) {
                        pageEl += ' - Top';
                    } else if (row.hasClass('add-to-row')) {
                        pageEl += ' - Bottom';
                    } else if (row.hasClass('top') && $el.hasClass('product-image')) {
                        pageEl = 'Product - Image Select';
                    } else if (row.hasClass('top')) {
                        pageEl = 'Product - Text Select';
                    }

                    BA_GAQ.manualTracker('event', $el, 'Comparison Page', pageEl, label, 'mousedown');
                });
            });
        });
